...
describe('stylelint', () => {
	...
	test('Value and Unit', async () => {
		...
		expect(violations).toStrictEqual([
			...
			'test/fixtures/stylelint/unit.scss:83:17 `ex`は使用しないでください。代わりに`em, rem`を検討してください。',
			'test/fixtures/stylelint/unit.scss:84:17 `pt`は使用しないでください。代わりに`px, rem`を検討してください。',
			'test/fixtures/stylelint/unit.scss:85:17 `cm`は使用しないでください。代わりに`px, rem`を検討してください。',
